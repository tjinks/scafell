...
static noreturn void raise_error(scf_err_info err_info) {
    if (err_handler) {
...
scf_err_info scf_err_info_create(scf_error_code code, const char *msg) {
    scf_err_info result = {code, false, 0};
...
#ifdef UNIX
    set_message(&result, strerror(code));
#endif
...
noreturn void scf_raise_fatal_error(scf_error_code code, const char *msg) {
    raise_error(scf_err_info_create(code, msg));
}
