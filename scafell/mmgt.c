...
static void default_exhaustion_handler(void) {
    scf_raise_fatal_error(SCF_OUT_OF_MEMORY, "Out of memory!");
}
...
void scf_buffer_insert_bytes(scf_buffer *buffer, const void *bytes_to_insert, size_t before, size_t byte_count) {
    if (before > buffer->size) scf_raise_fatal_error(SCF_BAD_INDEX, "Invalid index");
...
void scf_buffer_remove(scf_buffer *buffer, size_t starting_from, size_t byte_count) {
    if (starting_from + byte_count > buffer->size) scf_raise_fatal_error(SCF_BAD_INDEX, "Attempting to remove more data than is present!");
...
scf_buffer scf_buffer_extract(const scf_buffer *buffer, size_t starting_from, size_t byte_count) {
    if (starting_from + byte_count > buffer->size) scf_raise_fatal_error(SCF_BAD_INDEX, "Attempting to extract more data than is present!");
