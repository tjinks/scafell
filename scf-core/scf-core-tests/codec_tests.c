...
/*
 * The following are encodings (without null terminator) of the string 'A£<U1004D>'
 *
 * (U1004D = LINEAR B SYLLABLE B091 TWO)
 */
...
static const unsigned char utf32_le_encoded_test_data[] = {0x41, 0x00, 0x00, 0x00, 0xA3, 0x00, 0x00, 0x00, 0x4D, 0x00, 0x01, 0x00};
...
#define UTF32_LEN (sizeof(utf32_le_encoded_test_data) / sizeof(unsigned char))
...
static bool test_utf32_le_to_utf8(void) {
    scf_buffer source = scf_buffer_wrap((void *)utf32_le_encoded_test_data, UTF32_LEN);
    scf_buffer target = scf_buffer_create(&op, 0);
    size_t result = ucs_encode(&source, 0, UCS_UTF32 | UCS_LE, &target, UCS_UTF8);
    ASSERT_EQ(3, result);
...
static bool test_utf8_to_utf32_le(void) {
    scf_buffer source = scf_buffer_wrap((void *)utf8_encoded_test_data, UTF8_LEN);
    scf_buffer target = scf_buffer_create(&op, 0);
    size_t result = ucs_encode(&source, 0, UCS_UTF8, &target, UCS_UTF32 | UCS_LE);
    ASSERT_EQ(3, result);
    
    result = result && ASSERT_EQ(UTF32_LEN, target.size);
    result = result && ASSERT_EQ(0, memcmp(target.data, utf32_le_encoded_test_data, UTF32_LEN));
    return result;
}
...
TEST(test_utf32_le_to_utf8)
TEST(test_utf8_to_utf32_le)
END_TEST_GROUP
